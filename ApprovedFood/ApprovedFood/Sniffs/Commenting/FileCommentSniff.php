...
class ApprovedFood_Sniffs_Commenting_FileCommentSniff implements PHP_CodeSniffer_Sniff
{
    ...
    protected function processAuthors($errorPos)
    {
        ...
        if (empty($authors) === false) {
            ...
            } else if (preg_match('/^([a-zA-Z\s]+)(<[a-z\.]+@approvedfood.co.uk>)$/', $content) === 0) {
                $error = 'Expected form "Author Name <AuthorEmail@approvedfood.co.uk>" for author tag';
    ...
    protected function processCopyrights($errorPos)
    {
        ...
        if ($copyright !== null) {
            ...
            } else if (preg_match('/^Copyright \(c\) ([0-9]{4})(-[0-9]{4})? (Approved Food Ltd)$/', $content) === 0) {
                $error = 'Expected "Copyright (c) xxxx-xxxx Approved Food Ltd" for copyright declaration';
    ...
    protected function processLicense($errorPos)
    {
        ...
        if ($license !== null) {
            ...
            if (empty($url) === true && empty($content) === true) {
                ...
                } else if ($url !== 'http://approvedfood.biz') {
                    $error = 'Expected "http://approvedfood.biz" for license URL';
                ...
                if (empty($content) === true) {
                    ...
                    $error = 'Expected "Private" for license name';
