 ...
 */
class ApprovedFood_Sniffs_Styleguide_ClassesSniff implements PHP_CodeSniffer_Sniff
{
    /**
     * @var array
     */
        ...
        switch($current['code'])
        {
            ...
            case T_INTERFACE:
                ...
                if($nextClass !== false)
                {
                    ...
                    {
                        $phpcsFile->addWarning('Expected 1 blank line after a class.', $stackPtr);
                    }
                }
                if($next['code'] != T_WHITESPACE)
                {
                    $phpcsFile->addWarning('Space expected after class, interface', $stackPtr);
                }
                // find comment
                ...
                if(substr_count($className, 'Frontend') > 0 || substr_count($className, 'Backend') > 0 || substr_count($className, 'Api') > 0 || substr_count($className, 'Installer') > 0)
                {
                    ...
                    for($i = $startComment; $i <= $endComment; $i++)
                    {
                        ...
                        if(substr($tokens[$i]['content'], 0, 10) == ' * @author')
                        {
                            ...
                            {
                                $phpcsFile->addWarning('After @author there should be exactly one space.', $i);
                            }
                            // validate author format
                            $content = trim(substr($tokens[$i]['content'], strrpos($tokens[$i]['content'], ' ')));
                            if(preg_match('/.*<.*@.*>$/', $content) != 1)
                            {
                                $phpcsFile->addWarning('Invalid syntax for the @author-value', $i);
                            }
                        }
                    }
                    // no author found
                    if(!$hasAuthor)
                    {
                        $phpcsFile->addWarning('No author found in PHPDoc', $startComment);
                    }
                }
                break;
            case T_EXTENDS:
            case T_IMPLEMENTS:
                if($previous['content'] != ' ')
                {
                    $phpcsFile->addWarning('Space excpected before extends, implements', $stackPtr);
                }
                if($next['content'] != ' ')
                {
                    $phpcsFile->addWarning('Space expected after extends, implements', $stackPtr);
                }
                break;
            case T_FUNCTION:
                ...
                if(isset($tokens[$prevClass]['scope_closer']) && $current['line'] <= $tokens[$tokens[$prevClass]['scope_closer']]['line'])
                {
                    if($prevClass != 0)
                    {
                        ...
                        if(!in_array($className, self::$classesWithErrors))
                        {
                            ...
                            if(count($diff) > 0)
                            {
                                $phpcsFile->addWarning('The methods should be placed in alphabetical order.', $stackPtr);
